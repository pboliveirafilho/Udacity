...
```{r}
corrplot(cor(white_wine[2:13]), method = "number")
```
...
O *Primeiro gráfico* a classificação original da qualidade dos vinhos brancos dada pelos especialistas e a nova classificação de qualidade sugerida pelo autor no decorrer da análise:
...
ggplot(aes(x = new_quality, y = residual.sugar,
           fill = new_quality, color = new_quality),
       data = white_wine) +
  geom_violin(alpha = 0.5) +
  geom_boxplot(width=.1, color = "black") +
  scale_y_log10(name = "log10(Açúcar Residual) / (g.dm^-3)") +
  scale_x_discrete(name = "Qualidade") +
  ggtitle(label = "Violin Box Plot - Variação do Açúcar Residual") +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none")
```
...
O *Segundo Gráfico* mostra a variação da concentração do açúcar residual nos vinhos brancos classificados como *Ruim*, *Comum*, e *Bom*. O gráfico representa um das principais descobertas da *Seção de Gráficos Univariados*, o comportamento bimodal da concentração de açúcar residual revelado pela utilização da escala logarítmica.
...
ggplot(aes(x = density, y = alcohol, color = new_quality),
  ...
  facet_wrap(facets = ~new_quality) +
  scale_x_continuous(name = "Densidade / (g.cm^-3)", limits = c(0.985,1.005), breaks = seq(0.985,1.005,0.01)) +
